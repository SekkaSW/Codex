...
export class SupabaseRepositories extends FieldRepository implements Registry, TrailmarkStore, IntelStore, LedgerStore, StrongboxStore, AtlasGateway {
    ...
    }); return rows.map(row => ({ id: String(row.id), guildId: String(row.guild_id), amount: Number(row.amount), kind: String(row.kind) as LedgerEntry["kind"], actorId: String(row.actor_id), note: String(row.note), createdAt: String(row.created_at), ...(row.member_id ? { memberId: String(row.member_id) } : {}), ...(row.reversed_entry_id ? { reversedEntryId: String(row.reversed_entry_id) } : {}) })); }
    async append(row: LedgerEntry): Promise<void> { dataOrThrow(await this.client.from("fund_ledger").insert({ id: row.id, guild_id: row.guildId, amount: row.amount, kind: row.kind, actor_id: row.actorId, note: row.note, member_id:row.memberId??null, created_at: row.createdAt, reversed_entry_id: row.reversedEntryId ?? null })); }
