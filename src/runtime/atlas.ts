...
export async function handleAtlas(i:any,store:AtlasRepositories):Promise<void>{
 ...
 const status=await store.atlasRpc('codex_atlas',{p_guild:i.guildId,p_action:'status',p_member:i.user.id});await i.editReply({content:(status.link?'Atlas identity is linked.':'Atlas identity is not linked.')+(config.modules.trailmarks===false?' Trailmarks are not enabled for this group. Atlas linking remains available; Trailmark access, drops and polling are disabled.':''),files:[{attachment:Buffer.from(JSON.stringify(status,null,2)),name:'atlas-status.json'}]});
}
export class AtlasRuntime {
 ...
 async poll(guild:any):Promise<void>{
  const config=await this.store.load(guild.id);if(!config?.modules.atlas || config.modules.trailmarks===false)return;
