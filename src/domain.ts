...
export interface PermissionRole { guildId: string; roleId: string; tier: PermissionTier }
export interface Rank { id: string; guildId: string; name: string; roleId?: string; tier: PermissionTier }
export interface RankEdge { guildId: string; fromRankId: string; toRankId: string; branchId: string }
...
export function assignMember(existing: MemberAssignment[], next: MemberAssignment, entries: AssignmentEntry[], group: AssignmentGroup): MemberAssignment[] {
  ...
  if (!entry || entry.groupId !== group.id || next.guildId !== group.guildId) throw new Error("Assignment entry is not in this group");
  ...
  if (group.multiple) return [...withoutDuplicate, next];
