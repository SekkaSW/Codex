...
export interface StoredSetupDraft extends SetupDraft { guildId: string; ownerId: string; updatedAt: string; expiresAt: string }
export interface SetupDraftPersistence { loadSetupDraft(guildId:string):Promise<StoredSetupDraft|undefined>; saveSetupDraft(draft:StoredSetupDraft):Promise<void>; deleteSetupDraft(guildId:string,ownerId?:string):Promise<void> }
export async function resumeSetupDraft(store:SetupDraftPersistence,guildId:string,ownerId:string,existing?:ServerConfig,now=new Date()):Promise<StoredSetupDraft>{
  ...
  if(draft&&Date.parse(draft.expiresAt)<=now.getTime()){await store.deleteSetupDraft(guildId);draft=undefined;}
  if(draft&&draft.ownerId!==ownerId)throw new Error("Another administrator owns the active setup draft");
  if(draft)return draft;
  const initial=beginSetup(existing);draft={...initial,guildId,ownerId,updatedAt:now.toISOString(),expiresAt:new Date(now.getTime()+7*86400000).toISOString()};await store.saveSetupDraft(draft);return draft;
}
export async function cancelSetupDraft(store:SetupDraftPersistence,guildId:string,ownerId:string):Promise<void>{const draft=await store.loadSetupDraft(guildId);if(!draft)throw new Error("Start /server setup first");if(draft.ownerId!==ownerId)throw new Error("Only the draft owner may cancel setup");await store.deleteSetupDraft(guildId,ownerId);}
...
export function validateRankGraph(rankIds: Iterable<string>, edges: Array<{fromRankId:string;toRankId:string}>): void {
  const ids=new Set(rankIds);
  for(const edge of edges){
    if(edge.fromRankId===edge.toRankId)throw new Error("A rank cannot progress to itself");
    if(!ids.has(edge.fromRankId)||!ids.has(edge.toRankId))throw new Error("Rank progression references an unknown rank");
  }
}
