...
export interface ManagedResource {
    guildId: string;
    key: ResourceKey | `TRAILMARK:${string}` | `REPORT_TOPIC:${string}` | `EXTRA:${string}`;
    discordId: string;
    kind: "CATEGORY" | "CHANNEL" | "FORUM";
    ownerId?: string;
}
...
export interface Registry {
    list(guildId: string): Promise<ManagedResource[]>;
    put(resource: ManagedResource): Promise<void>;
}
export async function repairResources(guildId: string, specs: ResourceSpec[], registry: Registry, provisioner: Provisioner): Promise<{
    created: string[];
    retained: string[];
}> {
    const records = await registry.list(guildId);
