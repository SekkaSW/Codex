...
test('refined group and command use literal messages with normalization and natural confirmation', async () => { const f = fixture(); await f.start(); assert.match(f.last.content, /What is the name of the group/); await f.answer('Order'); assert.equal(f.d.config.organizationName, 'Order'); assert.match(f.last.content, /Group name set to/); await f.answer('/order'); assert.equal(f.d.config.commandNamespace, 'order'); assert.match(f.last.content, /Command set to \*\*\/order/); assert.match(f.last.content, /Permission levels are not ranks/); assert.equal(f.applied, 0); });
...
test('permission labels preserve internal meanings and native multi-role mappings', async () => { assert.deepEqual(Object.values(permissionLabels), ['Recruit', 'Member', 'Advanced Member', 'Advisors', 'Leader']); const f = fixture(); await f.start(); await f.at('permissions'); for (const [index, tier] of Object.keys(permissionLabels).entries()) {
    ...
    assert.equal(f.d.organization!.permissions.filter(p => p.tier === tier).length, 2);
...
} await f.act('answer', [b!.id]); await f.act('continue'); await f.act('answer', [a!.id]); assert.equal(f.d.organization!.edges.length, 1); assert.match(f.last.content, /loop/); });
...
test('Optional Features descriptions and multi-select retain other feature flags', async () => { const f = fixture(); await f.start(); f.d.config.modules!.intelligence = false; f.d.config.modules!.trailmarks = false; await f.at('features'); for (const text of Object.values(featureDescriptions))
    assert.ok(f.last.content.includes(text)); assert.doesNotMatch(f.last.content, /modules/i); await f.act('features', ['atlas', 'briefings']); assert.equal(f.d.config.modules!.intelligence, false); assert.equal(f.d.config.modules!.atlas, true); assert.match(f.last.content, /Enabled: Atlas, Briefings/); });
...
test('full refined message flow only creates duties and applies after final review confirmation', async () => { const f = fixture(); await f.start(); await f.answer('Order'); await f.answer('/order'); for (let n = 0; n < 5; n++)
    await f.act('continue'); await f.answer('A, B'); await f.act('continue'); for (const id of ['100', '101']) {
    await f.act('role', [id]);
    await f.act('continue');
} await f.act('no-branches'); const [a, b] = f.d.organization!.ranks; await f.act('answer', [a!.id]); await f.act('answer', [b!.id]); await f.act('continue'); await f.act('end'); await f.answer('Loremaster, Quartermaster'); await f.act('continue'); await f.act('no'); await f.act('features', ['atlas']); await f.act('continue'); await f.act('channel', ['commands']); await f.act('channel', ['logs']); await f.act('recommended'); await f.act('recommended'); await f.act('no'); await f.act('no'); assert.match(f.last.content, /Atlas enabled/); assert.equal(f.applied, 0); assert.equal(f.created, 0); await f.act('continue'); await f.act('confirm', [], true); assert.equal(f.applied, 1); assert.equal(f.created, 2); assert.equal(f.org.duties.length, 2); assert.equal(f.provisioned, 1); assert.equal(f.d, undefined); });
