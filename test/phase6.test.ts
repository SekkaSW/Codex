...
test('Phase 6 Discord modal persists forum thread and member selector; ownership and permission checks apply',async()=>{
 const {db,store}=await fixture();try{
  ...
  let output:any;const i:any={guild,guildId:'g',user:{id:'admin'},id:'modal-1',customId:'intel:admin:contact:create:new',fields:{getTextInputValue:(key:string)=>key==='name'?'Contact':'Description'},isModalSubmit:()=>true,async deferReply(){},async editReply(value:any){output=value;}};
