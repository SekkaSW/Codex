...
test('wizard reloads durable drafts; owner, expiry and stale-panel checks apply to components', async () => {
    ...
    let response:any;
    const interaction = (actor: string, revision: number, action: string) => ({ guildId: 'g', user: { id: actor }, memberPermissions: { has: () => true }, customId: `setup:${revision}:${action}`, values: ['permissions'], async reply(p:any) { response=p; }, async update(p:any) { response=p; }, async deferUpdate() { }, async editReply() { }, isModalSubmit: () => false });
    ...
    await wizard.handle(interaction('owner', 0, 'section'));
    assert.match(response.content,/changed/);assert.equal(response.components[0].components[0].label,'Resume');
