...
## Deploying the server setup refinement

Apply `migrations/012_server_setup_refinement.sql` after 011, or its generated mirror `supabase/migrations/20260919124748_server_setup_refinement.sql` using the existing Supabase migration history. Do not apply both copies to the same database. Prior migrations are unchanged. Existing guilds are backfilled with Intelligence and Trailmarks enabled; previously saved drafts retain values. Rebuild and restart the bot after migration. No slash-command redeployment is needed for this refinement; command definitions are unchanged.

The migration adds two keys to the existing feature table and a service-only, RLS-enabled duty creation receipt table/RPC. It amends existing RPC bodies without changing their signatures and retains the migration 011 hardening. Shared delivery receipts are explicitly exempt from the Intelligence gate because other systems use them. No anon/authenticated grants are added. Local tests cover a full fresh database and a populated upgrade, role grants, RLS, first-time duty audits, disable/re-enable preservation, and equality of both migration histories.

Creating duty roles requires **Manage Roles** and a bot role above the managed roles (not at the bottom of the role hierarchy). Defaults grant zero Discord permissions and do not hoist or make roles mentionable. **View Audit Log** is optional for exact recovery after an interrupted role-creation response. Without a stored result or matching bot audit event, creation stops instead of blindly retrying. An administrator can map the existing role in Detailed Editor using the same duty display name and confirm again. A deleted stored duty role similarly needs an explicit replacement mapping. Canceling setup after a partially completed Confirm does not delete roles already created; the durable receipts prevent accidental duplicates on retry.
