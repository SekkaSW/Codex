...
The wizard asks one question at a time, confirms each answer, and saves progress in an owner-scoped, seven-day Supabase draft. Free text uses actual messages from the setup owner; roles and destinations use Discord selectors. Codex creates or reuses a private setup thread when permitted. Otherwise it uses the starting channel and requires Discord Reply on the current question. Enable Message Content Intent in the Developer Portal. After a bot restart, run `/server setup` and choose Resume to continue saved progress. Back retains saved answers, optional questions offer Skip, and interrupted setup offers Resume, Start Over, and View Progress. Existing servers open an Edit Section menu, with a Detailed Editor for removals and advanced maintenance. Review includes a readable configuration download. Only Confirm Setup saves normalized configuration and its audit, provisions resources, and registers the organization namespace. Repair first shows a read-only health summary and asks for confirmation. Failed provisioning can be retried without replacing stored resource identities.
...
Destructive actions ask for confirmation. Expired input forms can be reopened from `/help`; setup progress remains durable across bot restarts. After updating the bot, redeploy slash commands as described in [deployment](docs/DEPLOYMENT.md). This setup correction requires a bot restart, but no slash-command redeployment or migration. See the [setup correction report](docs/SETUP_MESSAGE_CORRECTION.md) and [UX completion report](docs/UX_COMPLETION.md) for coverage and limitations.
