...
The wizard asks one question at a time, confirms each answer, and saves progress in an owner-scoped, seven-day Supabase draft. Free text uses single-field modals; roles and destinations use Discord selectors. Back retains saved answers, optional questions offer Skip, and interrupted setup offers Resume, Start Over, and View Progress. Existing servers open an Edit Section menu, with a Detailed Editor for removals and advanced maintenance. Review includes a readable configuration download. Only Confirm Setup saves normalized configuration and its audit, provisions resources, and registers the organization namespace. Repair first shows a read-only health summary and asks for confirmation. Failed provisioning can be retried without replacing stored resource identities.
